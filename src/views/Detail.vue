            ...
            <div class="col-12 col-md-6 col-lg-8">
              ...
              <div class="mt-4 gray-line">
                <div class="button-container d-flex gap-4">
                  <button v-if="!isOwner" class="btn btn-primary tez-btn flex-grow-1" id="like" @click="hitLike()"
                    :class="{ 'liked': this.isLiked }">
                    <span v-if="isLiked">Like</span>
                    <span v-else>Unlike</span>
                  </button>
                  <button v-else class="btn btn-primary tez-btn flex-grow-1"
                    @click="$router.push('/edit/' + this.$route.params.item)">Edit Post</button>
                  <button v-if="!isOwner" class="btn btn-primary tez-btn flex-grow-1" id="chat"
                    @click="ChatwithAuthor()">Chat</button>
                  <button v-else class="btn btn-primary tez-btn flex-grow-1" @click="confirmFinishPost"
                    :disabled="this.Blogs.sold">Finish Post</button>
                </div>
              </div>
              <div v-if="!isOwner" class="d-flex align-items-center mt-2 justify-content-center gap-2">
                <h5 class="mt-3">Send Offer</h5>
                <div class="input-group flex-fill">
                  <input type="text" placeholder="Offer Price" class="form-control flex-grow-1 inputOffer"
                    v-model="this.commentary" />
